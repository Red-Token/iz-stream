...
async function packageAfterCopy(
	...
	await Promise.all(
		[...config.builds, ...config.renderers].map((info) =>
			viteRun(['build', '-c', path.join(buildPath, info.configFile)], {cwd: buildPath, shell: true})
		)
	);
...
async function viteDevBuilder(
	...
	rendererServers: {name: string; server: vite.ViteDevServer; url: string}[]
...
async function viteDevRendererServer(
	info: Config['renderers'][number]
): Promise<{name: string; server: vite.ViteDevServer; url: string}> {
			...
			tasks: [
				{
					...
					task: async () => {
						rendererServers.push(
							...(await Promise.all(this.config.renderers.map((info) => viteDevRendererServer(info))))
						);
					},
				...
				},
				{
					...
					task: async () => {
						buildWatchers.push(
							...(await Promise.all(this.config.builds.map((info) => viteDevBuilder(info, rendererServers))))
						);
					},
					rendererOptions: {persistentOutput: true, timer: {...PRESET_TIMER}}
				}
