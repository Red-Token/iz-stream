...
export function nativeNodeFile(config: Config, root: string = process.cwd()): Plugin {
	const natives = config.map((native) => {
		const importFullPath = native.import === undefined ? undefined : path.resolve(root, native.import);

		const nodeFileFullPath = path.resolve(root, native.built);

		const requirePath = native.includePath === undefined ? `./${path.basename(nodeFileFullPath)}` : native.includePath;
