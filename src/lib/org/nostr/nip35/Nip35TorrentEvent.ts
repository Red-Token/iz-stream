...
export function createRefETags(event: TrustedEvent, relay: string = ''): string[][] {
	...
	if (eTags.length > 0) {
		...
		if (rootTag === undefined) throw new Error('not good, eTags found and no tag marked root');
...
export function createRefPTags(event: TrustedEvent, relay: string = ''): string[][] {
	return [...safeFindOptionalMultiTagValues(event, 'p'), ['p', event.pubkey]];
}
...
export class Nip35TorrentEventCommentsBuilder {
	...
	build() {
		return new Nip35TorrentEventComments(JSON.parse(this.event.content));
	}
}
