(() => {
	...
	var SHA2 = class extends Hash {
		...
		digestInto(out) {
			...
			for (let i2 = 0; i2 < outLen; i2++) oview.setUint32(4 * i2, state[i2], isLE4);
		}
		digest() {
	...
	function Field(ORDER, bitLen2, isLE4 = false, redef = {}) {
		...
		const f = Object.freeze({
			...
			fromBytes: (bytes4) => {
				if (bytes4.length !== BYTES) throw new Error(`Fp.fromBytes: expected ${BYTES}, got ${bytes4.length}`);
	...
	function radix(num) {
		...
		return {
			encode: (bytes4) => {
				if (!(bytes4 instanceof Uint8Array)) throw new Error('radix.encode input should be Uint8Array');
	...
	function radix2(bits, revPadding = false) {
		...
		return {
			encode: (bytes4) => {
				if (!(bytes4 instanceof Uint8Array)) throw new Error('radix2.encode input should be Uint8Array');
	...
	var swapLE = (n) =>
		(((n >>> 0) & 255) << 24) | (((n >>> 8) & 255) << 16) | (((n >>> 16) & 255) << 8) | ((n >>> 24) & 255) | 0;
	...
	function ctrCounter(xk, nonce, src, dst) {
		...
		if (start < srcLen) {
			...
			const buf = u8(b32);
			for (let i2 = start, pos = 0; i2 < srcLen; i2++, pos++) dst[i2] = src[i2] ^ buf[pos];
		}
		return dst;
	}
	...
	function validatePCKS(data, pcks5) {
		...
		for (let i2 = 0; i2 < lastByte; i2++)
			if (data[len - i2 - 1] !== lastByte) throw new Error(`aes/pcks5: wrong padding`);
		return out;
	}
	function padPCKS(left) {
		...
		for (let i2 = BLOCK_SIZE2 - paddingByte; i2 < BLOCK_SIZE2; i2++) tmp[i2] = paddingByte;
		return tmp32;
	}
	...
	var gcm = wrapCipher({blockSize: 16, nonceLength: 12, tagLength: 16}, function gcm2(key, nonce, AAD) {
		...
		return {
			...
			decrypt: (ciphertext) => {
				...
				if (!equalBytes2(tag, passedTag)) throw new Error('aes/gcm: invalid ghash tag');
	...
	var siv = wrapCipher({blockSize: 16, nonceLength: 12, tagLength: 16}, function siv2(key, nonce, AAD) {
		...
		return {
			...
			decrypt: (ciphertext) => {
				...
				encKey.fill(0);
				authKey.fill(0);
				if (!equalBytes2(tag, expectedTag)) throw new Error('invalid polyval tag');
				return plaintext;
			}
		};
	});
	...
	function createCipher(core, opts) {
		...
		return (key, nonce, data, output4, counter = 0) => {
			...
			if (output4.length < len) throw new Error(`arx: output (${output4.length}) is shorter than data (${len})`);
			const toClean = [];
			let l = key.length,
				k,
				sigma;
	...
	function expand(hash3, prk, info, length = 32) {
		...
		if (length > 255 * hash3.outputLen) throw new Error('Length should be <= 255*HashLen');
	...
	var _u = class {
		...
		static unpad(padded) {
			...
			if (
				unpaddedLen < _u.minPlaintextSize ||
				unpaddedLen > _u.maxPlaintextSize ||
				unpadded.length !== unpaddedLen ||
				padded.length !== 2 + _u.calcPaddedLen(unpaddedLen)
			)
	...
	async function getPosition(width2, height2) {
		...
		try {
			...
			if (
				lastFocused &&
				lastFocused.top !== void 0 &&
				lastFocused.left !== void 0 &&
				lastFocused.width !== void 0 &&
				lastFocused.height !== void 0
			) {
